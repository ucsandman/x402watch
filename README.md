...
Which paid APIs actually answer. Every x402 endpoint listed on the Coinbase Bazaar, the PayAI discovery catalog and x402scan, probed unpaid every six hours, ranked by whether it is reachable, charges what it declared, and how fast it answers.
...
1. `lib/crawl.ts` pulls every resource from the Bazaar and PayAI discovery APIs (same shape, offset paginated, no key) and x402scan (tRPC search), merged by URL.
...
Fix the listing at the source (your Bazaar discovery extension, PayAI discovery listing or x402scan registration). The next probe picks it up. Probes are one unpaid request each and identify as `x402watch/1`.
